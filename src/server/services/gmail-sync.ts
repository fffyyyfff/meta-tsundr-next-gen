...
export async function fetchPurchaseEmails(
  ...
  const afterStr = lastSyncAt
    ? ` after:${Math.floor(lastSyncAt.getTime() / 1000)}`
    : '';
  const q = `(from:auto-confirm@amazon.co.jp OR from:order@rakuten.co.jp)${afterStr}`;
  ...
  for (const msg of res.data.messages.slice(0, 20)) {
    try {
      ...
      if (htmlPart?.body?.data) {
        ...
        body = Buffer.from(detail.data.payload.body.data, 'base64url').toString();
      }
